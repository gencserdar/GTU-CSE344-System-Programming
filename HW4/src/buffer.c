...
int buffer_push(Buffer *buffer, BufferElement item) {
    ...
    while (buffer->count >= buffer->capacity) pthread_cond_wait(&buffer->full, &buffer->mutex);
...
int buffer_pop(Buffer *buffer, BufferElement *item) {
    pthread_mutex_lock(&buffer->mutex);
    while (buffer->count <= 0 && !buffer->done) pthread_cond_wait(&buffer->empty, &buffer->mutex);
...
int buffer_is_done(Buffer *buffer) {return buffer->done;}
