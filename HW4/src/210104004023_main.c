...
int main(int argc, char *argv[]) {
    ...
    ManagerArgs manager_args = {&buffer, source_directory, destination_directory};
    pthread_create(&manager_thread, NULL, manager_function, &manager_args);

    // Start worker threads
    for (int i = 0; i < worker_amount; i++) pthread_create(&worker_threads[i], NULL, worker_function, &buffer);
    
    // Wait for manager to finish
    pthread_join(manager_thread, NULL);

    // Wait for all workers to finish
    for (int i = 0; i < worker_amount; i++) pthread_join(worker_threads[i], NULL);
